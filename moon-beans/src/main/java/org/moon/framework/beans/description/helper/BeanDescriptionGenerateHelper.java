...
public class BeanDescriptionGenerateHelper implements BeanDescriptionGenerate {
	...
	public BeanDescription generate(Class<?> loadClass) {
		...
		if (null == aliasAnnotation) {
			aliases = new String[] { beanClassName };
		} else {
		...
		// Bean初始化时执行的函数
		Method[] initMethods = ReflectionUtils.getMethodByAnnotation(beanClass.getDeclaredMethods(), InitMethod.class);
		initMethods = ReflectionUtils.removeMethodArrayEmptyElement(initMethods);
		
		// Bean销毁时执行的函数
		Method[] destroyMethods = ReflectionUtils.getMethodByAnnotation(beanClass.getDeclaredMethods(),
				DestroyMethod.class);
		 destroyMethods = ReflectionUtils.removeMethodArrayEmptyElement(destroyMethods);
